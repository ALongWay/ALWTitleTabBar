...
{
    [self resetTitleTabBarWithTitleArray:titleArray configuration:[ALWTitleTabBarConfiguration defaultConfiguration]];
}
...
- (void)resetTitleTabBarWithTitleArray:(NSArray<NSString *> *)titleArray configuration:(ALWTitleTabBarConfiguration *)configuration
{
    ...
    CGFloat padding = configuration.contentViewHorizontalPadding;
    CGFloat originX = padding;
    CGFloat currentContentWidth = _labelTotalWidth + padding * 2;
    ...
    for (int i = 0; i < _bottomLabelArray.count; i++) {
        ...
        if (configuration.isEqualTitleViewWidth) {
            if (currentContentWidth < width) {
                //每个label平分容器宽度
                CGFloat equalWidth = (width - padding * 2) / titleArray.count;
                rect.size.width = equalWidth;
            }
        }else{
            if (currentContentWidth < width) {
                //如果内容总宽度小于tabBar宽度，将多出宽度平分到每个label
                CGFloat lastWidth = width - currentContentWidth;
    ...
    [_scrollView setContentSize:CGSizeMake(originX + padding, height)];
    _maskView.frame = CGRectMake(0, 0, originX + padding, height);
